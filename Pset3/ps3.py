...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        # Check if width, height and dirt_amount are valid
        if width <= 0:
            raise ValueError("Input width should be more than 0")
        if height <= 0:
            raise ValueError("Input height should be more than 0")
        if dirt_amount < 0:
            raise ValueError("Dirt amount cannot be less than 0")
        # Initialize a dict with key as tile coordinate and value as dirt_amount
        dirt_tile_dict = {}
        for x in range(width):
            for y in range(height):
                dirt_tile_dict[(x, y)] = dirt_amount
        self.width = width
        self.height = height
        self.dirt_amount = dirt_amount
        self.tile_dirt = dirt_tile_dict
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        # Get the rounded down coordinate of the pos
        x = math.floor(pos.get_x())
        y = math.floor(pos.get_y())
        # Update the dirt_amount for te tile pos is in
        self.tile_dirt[(x,y)] -= capacity
        # Prevents dirt_amount from going negative
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        # Loop through every tile. Increment counter for every tile and return counter
        clean_counter = 0
    ...
    def is_position_in_room(self, pos):
        ...
        """
        # Get x and y coor of pos, if within boundary, return True
        x = pos.get_x()
        y = pos.get_y()
...
class EmptyRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        Returns: True if pos is in the room, False otherwise.
        """
        # Get coordinates of pos. If within boundary, return True
        x = pos.get_x()
        y = pos.get_y()
        if (x < self.width and  x >= 0) and (y < self.height and  y >= 0):
            return True
    ...
    def get_random_position(self):
        ...
        """
        # Get a random positin within the constraint of room
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        # Get the tile the pos is on and check if tile is furnished
    ...
    def is_position_valid(self, pos):
        ...
        """
        # check if pos in room
        if self.is_position_in_room(pos) == False:
            return False
        # check if pos is furnished    
        if self.is_position_furnished(pos) == True:
            return False
        return True    
    ...
    def get_num_tiles(self):
        ...
        """
        # Loop through all tiles in room. Increment counter if tile is unfurnished
        accessible_tiles = 0
    ...
    def get_random_position(self):
        ...
        """
        # Get random coordinates. If coordinates are valid, return Position
        x = self.width * random.random()
        y = self.height * random.random()
        while self.is_position_valid(Position(x, y)) == False:
            x = self.width * random.random()
            y = self.height * random.random()
        return Position(x, y)    
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        # If position valid, update current position, clean the tile the new position is on
        if self.room.is_position_valid(new_position) == True:
            self.set_robot_position(new_position)
            self.room.clean_tile_at_position(new_position, self.capacity)
        # Else, change direction of robot    
        else:    
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        # Check if robot is faulty. If it is change its direction
        if self.gets_faulty() == True:
            self.set_robot_direction(random.uniform(0,360))   
        # Else get new position and clean current position
        else:
            new_position = self.get_robot_position().get_new_position(self.get_robot_direction(),self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                FaultyRobot)
    """
    # Initialize a list for the time taken for each trial
    time_taken_trial = []
    # For every trial, get the time step 
    for trial in range(num_trials):
        ...
        # Loop until min coverage is satisfied
        while float(room.get_num_cleaned_tiles()/room.get_num_tiles()) <= min_coverage:
            for robot in robots:
                robot.update_position_and_clean()
            time_step += 1
        time_taken_trial.append(time_step)
     # Return mean   
    return float(sum(time_taken_trial))/num_trials      
...
#       of a 20x20 room?
#   Time taken gets exponentially shorter for both robot types as the number of robots increase.
#   Time take for the standard robot is always shorter compared to the faulty robot for when number of robots used are equal
#   Differences in time between the cleanings using the standard and the fault robots get smaller as number of robots used increase
#
